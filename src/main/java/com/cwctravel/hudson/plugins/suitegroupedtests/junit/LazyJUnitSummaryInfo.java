	...
	private void computeSummary() {
		if(summary == null) {
			if(type == SUMMARY_TYPE_CLASS) {
				try {
					summary = junitDB.summarizeTestClassForBuild(testInfo.getBuildNumber(), testInfo.getProjectName(), testInfo.getSuiteName(), testInfo.getPackageName(), testInfo.getClassName());
				}
				...
				try {
					summary = junitDB.summarizeTestPackageForBuild(testInfo.getBuildNumber(), testInfo.getProjectName(), testInfo.getSuiteName(), testInfo.getPackageName());
				}
				catch(SQLException sE) {
					throw new JUnitException(sE);
				}
			}
			else if(type == SUMMARY_TYPE_SUITE) {
				try {
					summary = junitDB.summarizeTestSuiteForBuild(testInfo.getBuildNumber(), testInfo.getProjectName(), testInfo.getSuiteName());
				}
				catch(SQLException sE) {
					throw new JUnitException(sE);
				}
